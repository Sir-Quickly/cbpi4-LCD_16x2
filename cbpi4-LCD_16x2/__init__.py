...
class LCD_16x2(CBPiExtension):
    ...
    async def run(self):
        ...
        try:
            ...
            lcd.create_char(6, wheel)  # u"\x06"  -->wheel symbol
        ...
        logger.info('LCDisplay - xxxLCD single_kettle_id: %s' % single_kettle_id)

        sensor_for_sensor_mode = await self.set_lcd_sensortype_for_sensor_mode1()
        ...
        while True:
            ...
            if active_step != 'no active step' and display_mode == 'Multidisplay':
                if DEBUG: logger.info('LCDisplay - Multidisplay Screen shown')
                ...
                if DEBUG: logger.info('LCDisplay - Sensordisplay shown')
                sensortype = await self.set_lcd_sensortype_for_sensor_mode1()
    ...
    async def show_standby(self):
        ...
        if DEBUG: logger.info('LCDisplay - Standby Screen shown')
    ...
    async def show_singledisplay(self,kettle_id,charmap="A00",refresh_time=2.0):
        if DEBUG: logger.info('LCDisplay - SingleDIsplay shown')
        if DEBUG: logger.info('LCDisplay - Kettle ID. %s' %kettle_id )
        ...
        step_temperature = steps['active_step_target_temp']
        ...
        agitator = self.cbpi.actor.find_by_id(kettle.agitator)
        kettle_agitator_status = agitator.instance.state                            
        sensor_value = self.cbpi.sensor.get_sensor_value(kettle_sensor_id).get('value')

        if DEBUG: 
            logger.info('LCDisplay - Kettle %s' % kettle_name )
            logger.info('LCDisplay - Heater Status %s' % kettle_heater_status)
            logger.info('LCDisplay - Stirrer status %s' % kettle_agitator_status)
            logger.info('LCDisplay - Sensor Value: %s' % sensor_value)
            logger.info('LCDisplay - Step Name: %s' % step_name)
            logger.info('LCDisplay - Step Target Temperature: %s' % step_temperature)
            logger.info('LCDisplay - Remaining Time: %s' % remaining_time)
        
        #await asyncio.sleep(refresh_time)
     
        #line1 the stepname
        line1 = (u"%s" % kettle_name).ljust(16)[:16]

        if DEBUG:
            logger.info('LCDisplay - line1 %s' % line1)
        global line2_state
        if DEBUG:
        #    logger.info('LCDisplay - line1 %s' % line1)
            logger.info('LCDisplay - line2_state: %s' % str(line2_state))
        
        if line2_state == 0:
        #    if DEBUG: logger.info('LCDisplay - line2_state = 0')
            line2 = (u"Step: %s" % (step_name)).ljust(16)[:16]
            line2_state = 1
        elif line2_state == 1:
        #    if DEBUG: logger.info('LCDisplay - line2_state = 1')
            line2 = (u"Targ. T:%6.2f%s" % (float(step_temperature),(u"°C"))).ljust(16)[:16]
            line2_state = 2
        elif line2_state == 2:
        #    if DEBUG: logger.info('LCDisplay - line2_state = 2')
            try:
                line2 = (u"Curr. T:%6.2f%s" % (float(sensor_value),(u"°C"))).ljust(16)[:16]
            except:
                cbpi.app.logger.info("LCDDisplay  - singlemode current_sensor_value_id1 exception %s" % (current_sensor_value_id1))
                line2 = (u"Curr. T: %s" % (("No Data")))[:16]
            line2_state = 3
        else:
        #    if DEBUG: logger.info('LCDisplay - line2_state = something')
            ...
            if is_timer_running == True: #s.timer_end is not None:
                #time_remaining = time.strftime(u"%H:%M:%S", time.gmtime(s.timer_end - time.time()))
                line2 = (u"Time: %s" % (remaining_time).ljust(16))[:16]
            else:
        ...
        if DEBUG: logger.info('LCDisplay - Stirrer')
        if kettle_agitator_status != False:
            lcd.write_string(u"\x06")       
            STIRRER = 1
            #if DEBUG: logger.info('LCDisplay - Stirrer = 1')
        
        else:
            lcd.write_string(u" ")
            STIRRER = 0
            #if DEBUG: logger.info('LCDisplay - Stirrer = 0')
        ...
        lcd.cursor_pos = (1, 0)
    #   lcd.write_string (u"ß\x01\x00\x02\x03\x04\x05\x06")
        lcd.write_string(line2)
        await asyncio.sleep(refresh_time)

     
    async def show_singledisplay1(self, kettle_id, charmap="A00", refresh_time=1.0, multidisplay=False):
        ...
        try:
            ...
            while i < len(steps):
                if steps[i]["status"] == "A":
                    ...
                    active_step_type = (steps[i]["type"])
                    if active_step_type != "MashInStep":
                        active_step_timer_value = ((steps[i]["props"]["Timer"]))
                    else:
                        active_step_timer_value = "no_timer"
...
def setup(cbpi):
    cbpi.plugin.register("LCD_16x2", LCD_16x2)
